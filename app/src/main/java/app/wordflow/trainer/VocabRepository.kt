...
class VocabRepository(context: Context, private val db: AppDatabase) {
    ...
    suspend fun completeOnboarding(name: String, langIds: List<String>, goal: Int) {
        ...
        prefs.save {
            it.copy(
                onboardingDone = true,
                name = name.trim().ifBlank { it.cyloneName.ifBlank { "Nutzer" } }.take(24),
    ...
    suspend fun addWord(langId: String, chapterId: String, word: String, translation: String) {
        ...
        if (w.isBlank() || t.isBlank() || chapterId.isBlank()) return
        db.words().insertAll(
            listOf(
                WordEntity(
                    id = "manual-$langId-${System.currentTimeMillis()}",
                    lang = langId,
                    word = w,
                    translation = t,
                    chapterId = chapterId,
    ...
    suspend fun importPairs(lang: String, chapterId: String, pairs: List<VocabPair>) {
        ...
        val words = pairs.mapIndexed { index, pair ->
            WordEntity(
                id = "scan-$lang-$index-${System.currentTimeMillis()}",
                lang = lang,
                word = pair.word,
